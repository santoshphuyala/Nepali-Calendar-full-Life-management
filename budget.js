...
function showBudgetForm(budget = null) {
    ...
    // Initialize Nepali month picker for budget month
    setTimeout(() => {
        const monthInput = document.getElementById('budgetMonth');
        if (monthInput && !monthInput.hasAttribute('data-nepali-picker')) {
            new NepaliDatePicker(monthInput, {
                format: 'YYYY/MM',
                showWeekDays: false,
...
function showBillForm(bill = null) {
    ...
    // Initialize Nepali date picker for bill due date
    setTimeout(() => {
...
function showGoalForm(goal = null) {
    ...
    // Initialize Nepali date picker for goal target date
    setTimeout(() => {
...
async function deleteBudget(id) {
    ...
    try {
        ...
        if (currentView === 'budget') {
            const today = getCurrentNepaliDate();
            await updateBudgetOverview(today.year, today.month);
            await renderBudgetCategories(today.year, today.month);
            await renderBudgetChart(today.year, today.month);
        }
